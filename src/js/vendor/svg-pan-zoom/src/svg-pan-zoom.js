...
SvgPanZoom.prototype.resize = function() {
  ...
  if (options.fit || options.contain) {
    var newScale

    if (options.fit) {
      newScale = Math.min(options.width / viewbox.width, options.height / viewbox.height)
    } else {
      newScale = Math.max(options.width / viewbox.width, options.height / viewbox.height)
    }

    newCTM.a = newScale // x-scale
    newCTM.d = newScale // y-scale
    newCTM.e = -viewbox.x * newScale // x-transform
    newCTM.f = -viewbox.y * newScale // y-transform
  }

  if (options.center) {
    newCTM.e = (options.width - (viewbox.width + viewbox.x * 2) * newCTM.a) * 0.5
    newCTM.f = (options.height - (viewbox.height + viewbox.y * 2) * newCTM.a) * 0.5
  }
